...
function render(animate){boardEl.innerHTML='';for(let c=0;c<COLS;c++){const col=document.createElement('div');col.className='bcol '+(c%2?'odd':'even');for(let r=0;r<ROWS;r++){const cell=grid[r][c];const d=document.createElement('div');d.className='cell '+cellClass(cell);d.dataset.r=r;d.dataset.c=c;d.innerHTML=cellInner(cell);col.appendChild(d);}boardEl.appendChild(col);}
...
function startChain(p){if(grid[p.r][p.c].t==='stone')return;chain=[p];dragging=true;highlight();}
